...
hdfs_path = '/data/output.csv'#f'data/lazada_{timestamp}.csv'
...
df = (spark.read.format("com.databricks.spark.csv")
        ...
        .schema(schema)
        .load('hdfs://namenode:9000/data/output.csv'))

df.show()
#
filtered_df = df.select(['NAME', 'originalPrice','DiscountedPrice','Discount', 'ratingScore', 'review', 'categories', 'itemSoldCntShow', 'sellerName', 'sellerId','brandId','brandName','location'])
...
pandas_df.to_sql('test1', engine, if_exists='append', index=False)
